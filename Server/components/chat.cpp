...
liboai::Response liboai::ChatCompletion::create(const std::string& model, const Conversation& conversation, std::optional<float> temperature,
	std::optional<float> top_p, std::optional<uint16_t> n, std::optional<std::function<bool(std::string, intptr_t)>> stream,
	std::optional<std::vector<std::string>> stop, std::optional<uint16_t> max_tokens, std::optional<float> presence_penalty,
	std::optional<float> frequency_penalty, std::optional<std::unordered_map<std::string, int8_t>> logit_bias,
	std::optional<std::string> user, std::optional<std::intptr_t> param) const& noexcept(false) {
	...
	res = this->Request(
		...
		stream ? netimpl::components::WriteCallback{ std::move(stream.value()),param.value()} : netimpl::components::WriteCallback{},
